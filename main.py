...
def ptable_heatmap_mpl(vals_dict: dict, site: str, stype: str, cmap: str, title=None):
    ...
    def safe_format_formula(formula):
        try:
            ...
            if formula.startswith("(") and formula.endswith(")"):
                formula = formula[1:-1]
            # If it looks like a mixed element formula (e.g. Cr0.16Mo0.38Co0.46)
    ...
    if site is not None:
        text_str = f"element distribution of {site} site in {safe_format_formula(stype.split(',')[0])}-type compounds"
    else:
        if title is not None:
            text_str = title
        else:
            text_str = f"element distribution in {safe_format_formula(stype.split(',')[0])}-type compounds"
...
def format_mixed_formula(formula):
    """
    Converts a string like 'Cr0.16Mo0.38Co0.46' to 'Cr 0.16, Mo 0.38, Co 0.46'
    """
...
if __name__ == "__main__":
    ...
    # Find all folders in the script directory that contain at least one .cif file
    candidate_dirs = []
    for d in os.listdir(script_dir):
        full_path = os.path.join(script_dir, d)
        if os.path.isdir(full_path):
            cif_files = [f for f in os.listdir(full_path) if f.lower().endswith('.cif')]
            if cif_files:
                candidate_dirs.append((d, cif_files))

    if not candidate_dirs:
        print(f"No folders containing .cif files found in {script_dir}. Please add a folder with .cif files.")
        exit(0)
    ...
    for idx in selected_indices:
        ...
        cif_dir = os.path.join(script_dir, dir_name)
        print(f"\nProcessing folder: {dir_name}")

        # --- The rest of your original processing code goes here, replacing 'cif_dir' and 'cif_filenames' as needed ---
        cif_file_data = []
        ...
        shared = find_sites_with_same_wyckoff_symbols(os.path.join(cif_dir, selected_entries[0]))
        data0 = get_data_row(os.path.join(cif_dir, selected_entries[0]), shared)
        ...
        data[columns].to_csv(f"{'-'.join(selected_stype.split(',')[:2])}_{dir_name}.csv", index=False)
        ...
        for i, site in enumerate(sites):
            ...
            filename = f"ElemDist_{'-'.join(selected_stype.split(',')[:2])}_{site.replace('$', '').replace(' ', '')}.png"
            png_files.append(filename)

        # Cumulative heatmap
        ptable_heatmap_mpl(vals_dict=get_site_element_dist(data, site=None), site=None, stype=selected_stype, cmap="Greys")
        cum_filename = f"ElemDist_{'-'.join(selected_stype.split(',')[:2])}.png"
        png_files.append(cum_filename)

        csv_filename = f"{'-'.join(selected_stype.split(',')[:2])}_{dir_name}.csv"
